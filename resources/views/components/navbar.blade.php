<nav x-data="{ open: false }" class="bg-white shadow-sm">
            ...
            <div class="flex">
                ...
                <div class="hidden sm:ml-6 sm:flex">
                    ...
                    @foreach($items as $link => $url)
                        <a href="{{$url}}"
                           class="@if(!$loop->first) ml-8 @endif inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out">
                            {{$link}}
                        </a>
                    @endforeach
                </div>
                ...
                <div @click.away="open = false" class="ml-3 relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                        ...
                        <div class="py-1 rounded-md bg-white shadow-xs">
                            <a href="/user/profile"
                               class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out">Dein
                                Profil</a>
                            <a href="/docs" target="_blank"
                               class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out">Dokumentation</a>
                            <a href="#"
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3">
            ...
            @foreach($items as $link => $url)
                <a href="{{$url}}"
                   class="@if(!$loop->first) mt-1 @endif block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out">
                    {{$link}}
                </a>
            @endforeach
        </div>
